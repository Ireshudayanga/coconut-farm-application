...
export default function DailyUpdateDashboard() {
  ...
  const filteredUpdates = updates.filter((item) => {
    ...
    const matchesFlags = activeFlags.length === 0 || activeFlags.every((flag) => item.flags?.includes(flag));
    ...
    <div className="min-h-screen bg-white text-gray-900 p-4 sm:p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-4">
          {/* Tree ID */}
          <div>
            <label className="block text-sm text-gray-600 mb-1">Tree ID</label>
            <input
              type="text"
              value={searchId}
              onChange={(e) => setSearchId(e.target.value)}
              placeholder="Search by Tree ID"
              className="w-full bg-white text-gray-900 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          </div>

          {/* Watered */}
          <div>
            <label className="block text-sm text-gray-600 mb-1">Watered?</label>
            <select
              value={wateredFilter}
            ...
            filteredUpdates.map((update, i) => (
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-green-700 text-lg">{update.treeId}</span>
                ...
                <div className="flex items-center gap-2 text-sm">
                  <span
                    className={`px-2 py-1 rounded text-xs font-semibold ${update.watered ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                  >
