...
import '@/app/globals.css'; // Ensure safe area classes are available
...
export default function OwnerLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-950 text-white">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen w-64 bg-gray-900 border-r border-gray-800 p-6 z-40 transform transition-transform duration-300 md:static md:translate-x-0 ${
      ...
      {/* Main content */}
      <main className="flex-1 pt-14 p-4 sm:p-6 md:ml-64 w-full">{children}</main>
    </div>
  );
}
